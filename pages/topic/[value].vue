        ...
        <div class="flex flex-col items-center">
            <div class="lg:w-3/4 xl:w-6/12">
                <div class="flex flex-row bg-white mt-3 rounded-md shadow-sm animate-fadeIn30">
                    ...
                    <NuxtLink :to="'/topic/' + value" class="flex justify-center items-center text-gray-500">
                ...
                <div class="flex justify-start flex-wrap mt-4 animate-fadeIn30">
                    <Spin :show="loading">
                        <div v-for="(post, v) in topicPostList" :key="v" class="w-1/6 p-1.5">
...
import { TOPICPOSTLIST } from '~~/config/api';
import { TopicPostList } from '~~/types/response';
...
function nextPage() {
    page.value++
    getTopicPostList((data) => topicPostList.value.push(...data.value.list.post))
}

function sortPost(type: String, descs: boolean) {
    orderKey.value = type
    desc.value = descs
    getTopicPostList((data) => topicPostList.value = data.value.list.post)
}

async function getTopicPostList(f1: Function) {
    loading.value = true
    const { result } = await useCustomFetch<TopicPostList>(TOPICPOSTLIST, {
        method: 'GET',
        params: {
...
const title = computed(() => result.value.list.label)
const value = computed(() => result.value.list.value)
topicPostList.value.push(...result.value.list.post)
loading.value = false
